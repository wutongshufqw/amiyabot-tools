...
file_path = curr_path / 'config.json'
...
def save_f(data: Dict[str, Any]) -> bool:
    data = json.dumps(data, ensure_ascii=False, indent=3)
    save_mode = 'w' if file_path.exists() else 'x'
    with open(file_path, save_mode, encoding='utf8') as f:
        f.write(data)
    return True
...
def get_config():
    try:
        ...
        log.warning(f'宗门配置更新失败, 即将覆写: {e}')
        config = CONFIG
        save_f(config)
    return config


sect_config = get_config()
