...
class TrieHandle:
    ...
    def find_handle(self, data: Message) -> Union[Command, None]:
        try:
            index = next((i for i, mes in enumerate(data.message.get('message'))
                          if mes.get('type') == "text" and mes.get('data')['text'].strip()), -1)
        except AttributeError:
            index = next((i for i, mes in enumerate(data.message_raw.get('message'))
                          if mes.get('type') == "text" and mes.get('data')['text'].strip()), -1)
        ...
        prefix = data.message.get('message')[index].get('data')["text"].lstrip()
        handle = self.find(prefix)
        if not handle:
            return None

        prefix = prefix[len(handle.key):]
        if not prefix and len(data.message.get('message')) > 1:
            del data.message.get('message')[index]
        else:
            data.message.get('message')[index].get('data')['text'] = prefix
