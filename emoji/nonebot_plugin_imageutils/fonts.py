...
def add_font_to_manager(path: Union[str, Path]):
    try:
        ...
        log.warning(f"Failed to open font file {path}: {exc}")
    except Exception as exc:
        log.warning(f"Failed to extract font properties from {path}: {exc}")
...
def get_proper_font(
    ...
    log.warning(f"在当前字体列表中找不到可以显示字符“{char}”的字体")
...
async def download_font(url: str, fontpath: Path):
    ...
    async with httpx.AsyncClient() as client:
        async with client.stream("GET", url) as resp:
            log.info(f"Begin to download font from {url}")
